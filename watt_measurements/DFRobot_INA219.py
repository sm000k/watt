...
def wattmeter_name(argument):
    # only four addresses are available. more devices than 4 will require i2c a multiplexer
    switcher = {
        0x40: "wattmeter_1",
        0x41: "wattmeter_2",
        0x44: "wattmeter_3",
...
class INA219:
    ...
    INA219_I2C_ADDRESS5 = (0x46)
    INA219_I2C_ADDRESS6 = (0x49)
    ...
    def __init__(self, bus, addr):
        ...
        self.i2c_addr = addr
        self.name = wattmeter_name(addr)

    def begin(self):

        # moje wziete z dokumetnacji biblioteki w c++
        adc_bits_9 = 0
        adc_bits_10 = 1
        adc_bits_11 = 2
        adc_bits_12 = 3
        ...
        if not self.scan():
            ...
            self.set_mode(self.shunt_and_bus_vol_con)
            return True
    def set_steamjet_measurement_parameters(self):
        ina219_reading_mA = 1000
        ext_meter_reading_mA = 1000
        self.linear_cal(ina219_reading_mA, ext_meter_reading_mA)
        self.set_bus_RNG(self.bus_vol_range_16V)
        self.set_PGA(self.PGA_bits_2)
        self.set_bus_ADC(self.adc_bits_12, self.adc_sample_1)  # był adc_samble_8
        self.set_shunt_ADC(self.adc_bits_12, self.adc_sample_1)  # było adc_sample_8
        self.set_mode(self.shunt_and_bus_vol_con)
    ...
    def get_I_V_P (self):
        return [ float(self.read_ina_reg(self._INA219_REG_CURRENT)), float(self.read_ina_reg(self._INA219_REG_BUSVOLTAGE) >> 1) * 0.001,float(self.read_ina_reg(self._INA219_REG_POWER)) * 20]
    ...
    def scan(self):
        try:
            ...
            print("I2C init fail, no connection or wrong i2c address", self.name)
            return False
